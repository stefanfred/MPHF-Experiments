...
void rustPHastPlusContenderRunner(size_t N, bool use_ef) {
    ...
    for (size_t bucket_size_100 = 430; bucket_size_100 <= 570; bucket_size_100 += 30)
        RustPhastPlusContender(N, 9, bucket_size_100, use_ef).run();

    for (size_t bucket_size_100 = 440; bucket_size_100 <= 620; bucket_size_100 += 30)
        RustPhastPlusContender(N, 10, bucket_size_100, use_ef).run();

    for (size_t bucket_size_100 = 480; bucket_size_100 <= 680; bucket_size_100 += 30)
        RustPhastPlusContender(N, 11, bucket_size_100, use_ef).run();

    for (size_t bucket_size_100 = 470; bucket_size_100 <= 715; bucket_size_100 += 30)
